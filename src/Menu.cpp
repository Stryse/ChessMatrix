...
{
     std::cout << "================= ChessMatrix Program Menu =================" << std::endl << std::endl;
...
void Menu::init()
{
    ...
    {
        printOptions();
        std::cout << std::endl << ">>>> " ; std::cin >> choice;
        switch (choice)
        {
            case 0: std::cout << "Exiting..." << std::endl; break;
            case 1: printMatrixOne(); break;
            case 2: printMatrixTwo(); break;
            ...
            default: 
                    std::cout << "Menu point not found, exiting..." << std::endl;
                    continue;
            break;
        }
    } while(choice != 0);
...
void Menu::printMatrixOne()
{
    std::cout << *m_chMatrix_1;
}

void Menu::printMatrixTwo()
{
    std::cout << *m_chMatrix_2;
}
        ...
        switch (excpt)
            {
                case ChessMatrix::ChessMatrixExceptions::INDEX_OUT_OF_BOUNDS:
                    std::cout << "Overindexing!" << std::endl; return; break;
        
                default:
                    std::cout << "Unhandled exception" << std::endl; return; break;
            }
        }
